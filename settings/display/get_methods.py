...
def get_current_settings():
    current_orientation = os.popen("xrandr --verbose --screen 0 | grep -oP '\) [a-z]+ \('").read()
    ...
    current_settings = os.popen("xrandr --screen 0 | grep -oP '[0-9]+x[0-9]+[+][0-9][+][0-9]'").read()
    current_settings = current_settings.rstrip().split('+')
    return current_settings[0].rstrip(), current_orientation

def get_resolutions():
    list_of_resolutions = os.popen('xrandr --screen 0 | grep -oP " [0-9]+x[0-9]+ +[0-9.*+ ]+"').read()
